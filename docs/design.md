...
开发一个基于 Next.js 的个人导航页应用，支持从浏览器书签（HTML 文件）或当前浏览器页面快速导入链接。用户可以整理常用链接，并扩展“正在看”和“稍后看”两个动态列表。同时支持添加第三方组件（如 Trello 电子看板嵌入），数据全部存储在后端本地 JSON 文件中，可部署到任意云服务器（VPS）。
